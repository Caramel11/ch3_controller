...
from src.robot_interface import (
    ...
    compute_torque_no_rcm, compute_u_rotation, fixed_downward_tool_euler,
...
def _ros2_control_controller_state(name):
    proc = subprocess.run(
...
def _run_retreat_cmd(cmd, timeout=20.0):
    rospy.loginfo("  retreat controller command: " + " ".join(cmd))
...
def retreat_with_franka_move_to_start(robot, timeout=20.0):
    """Use franka_ros2's MoveToStartExampleController for the retreat phase."""
    if not hasattr(robot, "spin_once"):
        return False

    controller = "move_to_start_example_controller"
    no_rcm_controller = "no_rcm_effort_controller"
    ...
    if _ros2_control_controller_state(controller) is None:
        proc = _run_retreat_cmd(
            [
                "ros2",
                "run",
                "controller_manager",
        ...
        if proc.returncode != 0:
            rospy.logwarn(proc.stdout[-1200:])
            return False

    subprocess.run(
        ["ros2", "param", "set", f"/{controller}", "process_finished", "false"],
        check=False,
        text=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
    ...
    deactivate = []
    for candidate in (no_rcm_controller, gravity_controller):
        if _ros2_control_controller_state(candidate) == "active":
            deactivate.extend(["--deactivate", candidate])
    switch_cmd = [
        "ros2",
        "control",
        "switch_controllers",
    ...
    proc = _run_retreat_cmd(switch_cmd, timeout=15.0)
    if proc.returncode != 0:
        rospy.logwarn(proc.stdout[-1200:])
        return False

    target = np.asarray(INIT_JOINTS, dtype=float)
    ...
    while not rospy.is_shutdown() and time.time() < deadline:
        ...
        if now - last_log >= 1.0:
            last_log = now
            rospy.loginfo(
                f"  retreat built-in: err_inf={err_inf:.4f}rad, "
                f"err_norm={err_norm:.4f}rad, qd_norm={qd_norm:.4f}rad/s"
            )
        if err_inf <= 0.035 and qd_norm <= 0.10:
            rospy.loginfo("  retreat built-in: reached INIT_JOINTS.")
            return True
        time.sleep(0.01)

    rospy.logwarn("  retreat built-in: timeout before reaching INIT_JOINTS.")
    return False


def ensure_no_rcm_effort_controller_active():
    """Switch Gazebo back to the no-RCM effort controller for custom retreat."""
    ...
    state = _ros2_control_controller_state(controller)
    if state is None:
        proc = _run_retreat_cmd(
            [
                "ros2",
    ...
    deactivate = []
    for candidate in (move_to_start_controller, gravity_controller):
        if _ros2_control_controller_state(candidate) == "active":
            deactivate.extend(["--deactivate", candidate])
    switch_cmd = [
        "ros2",
    ...
    if proc.returncode != 0:
        rospy.logwarn(proc.stdout[-1200:])
        return False
    return _ros2_control_controller_state(controller) == "active"
...
def smoothstep5_dot(s):
    s = float(np.clip(s, 0.0, 1.0))
    return 30.0 * s ** 2 - 60.0 * s ** 3 + 30.0 * s ** 4


def _save_controller_attrs(ctrl, names):
    saved = {}
    missing = object()
    for name in names:
        saved[name] = getattr(ctrl, name, missing)
    return saved, missing


def _restore_controller_attrs(ctrl, saved, missing):
...
def retreat_with_position_only_controller(
    robot, kin_tool, kin_flange, ctrl, target_pos, ref_euler_fixed, cfg, timeout=20.0
):
    """Custom Cartesian retreat with approach-like staged reference tracking.

    The retreat phase intentionally avoids force tracking.  It first lifts the
    tool only enough to clear contact, then returns along a 3D Cartesian path to
    the saved home pose while holding the down-forward attitude.  The reference
    only advances while the real tool is following it, which keeps the phase
    fast without building a large Cartesian error that later excites the wrist.
    """
    del ctrl
    if not hasattr(robot, "exec_torque_cmd"):
        return False
    if hasattr(robot, "wait_until_ready") and not robot.wait_until_ready(timeout=2.0):
        rospy.logwarn("  retreat position-only: no fresh robot state.")
        return False

    rs0 = update_robot_state(kin_tool, kin_flange)
    start_pos = np.asarray(rs0["tool_position"], dtype=float).copy()
    target_pos = np.asarray(target_pos, dtype=float).reshape(3)
    retreat_ref = start_pos.copy()
    if target_pos[2] >= start_pos[2]:
        clear_z = min(
            float(target_pos[2]),
            float(start_pos[2] + cfg.cartesian_retreat_clearance),
        )
    else:
        clear_z = float(target_pos[2])
    timeout = max(float(timeout), 8.0)

    kp = np.asarray(cfg.cartesian_retreat_kp, dtype=float).reshape(3)
    kd = np.asarray(cfg.cartesian_retreat_kd, dtype=float).reshape(3)
    u_tool_limits = np.asarray(cfg.cartesian_retreat_u_tool_limits, dtype=float).reshape(3)

    rate = rospy.Rate(cfg.ctrl_rate)
    integ_euler = np.zeros(3)
    tau_limiter = VectorRateLimiter(cfg.cartesian_retreat_tau_rate_limit)
    t0 = time.time()
    last_step_time = t0
    last_log = 0.0
    settle_since = None
    ...
    ok = False
    rospy.loginfo(
        f"  retreat staged position-only PD: start={start_pos}, target={target_pos}, "
        f"clear_z={clear_z:.4f}m, "
        f"z_speed={cfg.cartesian_retreat_z_speed:.4f}m/s, "
        f"return_speed={cfg.cartesian_retreat_return_speed:.4f}m/s, "
    ...
    try:
        while not rospy.is_shutdown() and time.time() - t0 < timeout:
            ...
            if step_dt <= 0.0 or not np.isfinite(step_dt):
                step_dt = cfg.dt
            step_dt = min(max(step_dt, cfg.dt), 0.1)
            last_step_time = now

            rs = update_robot_state(kin_tool, kin_flange)
            tp = rs["tool_position"]
            tv = rs["tool_position_velocity"]
            prev_ref = retreat_ref.copy()
            follow_err = float(np.linalg.norm(tp - retreat_ref))
            speed_scale = retreat_speed_scale(follow_err, cfg)

            xy_err_actual = float(np.linalg.norm(tp[:2] - target_pos[:2]))
            z_err_actual = abs(float(tp[2] - target_pos[2]))
            if stage == "clear_contact":
                retreat_ref[:2] = start_pos[:2]
                z_step = cfg.cartesian_retreat_z_speed * speed_scale * step_dt
                z_target = clear_z
                z_delta = z_target - float(retreat_ref[2])
                if abs(z_delta) <= z_step:
                    z_candidate = z_target
                else:
                    z_candidate = float(retreat_ref[2]) + np.sign(z_delta) * z_step
                if z_target >= start_pos[2]:
                    z_cap = float(tp[2]) + cfg.cartesian_retreat_z_lag_limit
                    retreat_ref[2] = min(z_target, z_candidate, z_cap)
                else:
                    z_cap = float(tp[2]) - cfg.cartesian_retreat_z_lag_limit
                    retreat_ref[2] = max(z_target, z_candidate, z_cap)

                ref_z_done = abs(float(retreat_ref[2] - clear_z)) <= cfg.cartesian_retreat_z_tolerance
                tool_clearance = abs(float(tp[2] - start_pos[2]))
                tool_z_done = (
                    abs(float(tp[2] - clear_z)) <= cfg.cartesian_retreat_clear_z_tolerance
                    or tool_clearance >= cfg.cartesian_retreat_min_clearance
                )
                if ref_z_done and tool_z_done:
                    stage = "return_home"
                    settle_since = None
            elif stage == "return_home":
                retreat_ref = slew_toward(
                    retreat_ref,
                    target_pos,
                    cfg.cartesian_retreat_return_speed * speed_scale * step_dt,
                )
                ref_done = (
                    np.linalg.norm(retreat_ref - target_pos)
                    <= cfg.cartesian_retreat_position_tol
                )
                target_err_actual = float(np.linalg.norm(tp - target_pos))
                if ref_done and target_err_actual <= cfg.cartesian_retreat_position_tol:
                    stage = "settle"
                    settle_since = None
            else:
                retreat_ref = target_pos.copy()

            x_ref = retreat_ref.copy()
            xdot_ref = (retreat_ref - prev_ref) / step_dt
            e_r1 = tp - x_ref
            e_r2 = tv - xdot_ref
            if np.all(np.asarray(cfg.cartesian_retreat_position_error_limit) > 0.0):
                e_r1 = np.clip(
                    e_r1,
                    -cfg.cartesian_retreat_position_error_limit,
                    cfg.cartesian_retreat_position_error_limit,
                )
            if np.all(np.asarray(cfg.cartesian_retreat_velocity_error_limit) > 0.0):
                e_r2 = np.clip(
                    e_r2,
                    -cfg.cartesian_retreat_velocity_error_limit,
                    cfg.cartesian_retreat_velocity_error_limit,
                )

            kp_eff = kp
            kd_eff = kd
            if float(np.linalg.norm(x_ref - target_pos)) <= cfg.cartesian_retreat_final_zone:
                kp_eff = kp * cfg.cartesian_retreat_final_kp_scale
                kd_eff = kd * cfg.cartesian_retreat_final_kd_scale
            u_tool = -(kp_eff * e_r1 + kd_eff * e_r2)
            u_tool = np.clip(u_tool, -u_tool_limits, u_tool_limits)
            u_rot, integ_euler = compute_u_rotation(
                rs["tool_rotation_euler"],
                rs["tool_rotation_euler_velocity"],
                ref_euler_fixed,
                integ_euler,
                cfg.dt,
                P_ori=cfg.cartesian_retreat_P_ori,
                D_ori=cfg.cartesian_retreat_D_ori,
                I_ori=0.0,
                omega_limit=cfg.cartesian_retreat_omega_limit,
            )
            rot_norm = float(np.linalg.norm(u_rot))
            if rot_norm > cfg.cartesian_retreat_u_rot_limit > 0.0:
                u_rot = u_rot * (cfg.cartesian_retreat_u_rot_limit / rot_norm)
            u_cart = np.hstack([u_tool, u_rot])
            cart_norm = float(np.linalg.norm(u_cart))
            if cart_norm > cfg.cartesian_retreat_u_threshold > 0.0:
                u_cart = u_cart * (cfg.cartesian_retreat_u_threshold / cart_norm)
            J_tool = np.asarray(kin_tool.jacobian(), dtype=float)
            tau = (J_tool.T @ u_cart).flatten()
            qd = np.asarray(getattr(robot, "_qd_meas", np.zeros(7)), dtype=float).reshape(-1)
            if qd.size == tau.size and cfg.cartesian_retreat_joint_damping > 0.0:
                tau_damp = -cfg.cartesian_retreat_joint_damping * qd
                tau_damp = np.clip(
                    tau_damp,
                    -cfg.cartesian_retreat_joint_damping_tau_max,
                    cfg.cartesian_retreat_joint_damping_tau_max,
                )
                tau = tau + tau_damp
            tau = clamp_norm(tau, cfg.cartesian_retreat_tau_norm_limit)
            if cfg.cartesian_retreat_tau_rate_limit > 0.0:
                tau = tau_limiter.update(tau, cfg.dt)
            robot.exec_torque_cmd(tau)

            posture = orientation_diagnostics(
                robot, rs, ref_euler_fixed, cfg.no_rcm_nominal_joints
            )
            pos_err = float(np.linalg.norm(tp - target_pos))
            qd_norm = float(posture["qd_norm"])
            ori_err_deg = float(np.degrees(np.linalg.norm(posture["ori_err_rotvec"])))
            stable = (
                stage == "settle"
                and pos_err <= cfg.cartesian_retreat_position_tol
                and qd_norm <= cfg.cartesian_retreat_velocity_tol
                and ori_err_deg <= cfg.cartesian_retreat_orientation_tol_deg
            )
            if stable:
                if settle_since is None:
                    settle_since = now
                if now - settle_since >= cfg.cartesian_retreat_settle_time:
                    ok = True
                    rospy.loginfo(
                        f"  retreat position-only: reached target "
                        f"(pos_err={pos_err*1000:.2f}mm, "
                        f"ori={ori_err_deg:.2f}deg, "
                        f"front={posture['front_axis_err_deg']:.2f}deg, "
                        f"qd={qd_norm:.4f}rad/s)."
                    )
                    break
            else:
                settle_since = None
            ...
            if now - last_log >= 1.0:
                last_log = now
                rospy.loginfo(
                    f"  retreat staged PD: t={elapsed:.2f}s/{timeout:.2f}s, "
                    f"stage={stage}, "
                    f"pos_err={pos_err*1000:.2f}mm, "
                    f"xy={xy_err_actual*1000:.2f}mm, z={z_err_actual*1000:.2f}mm, "
                    f"ref=[{x_ref[0]*1000:.1f},{x_ref[1]*1000:.1f},{x_ref[2]*1000:.1f}]mm, "
                    f"tool=[{tp[0]*1000:.1f},{tp[1]*1000:.1f},{tp[2]*1000:.1f}]mm, "
                    f"follow={follow_err*1000:.2f}mm, scale={speed_scale:.2f}, "
                    f"u=[{u_tool[0]:+.2f},{u_tool[1]:+.2f},{u_tool[2]:+.2f}]N, "
                    f"ori={ori_err_deg:.2f}deg, "
                    f"front={posture['front_axis_err_deg']:.2f}deg, "
                    f"qd={qd_norm:.4f}rad/s"
                )
        ...
        # Hold the final pose with gravity compensation only briefly before the caller exits.
        if ok:
            hold_deadline = time.time() + 0.25
            while time.time() < hold_deadline and not rospy.is_shutdown():
                robot.exec_torque_cmd(np.zeros(7))
                time.sleep(0.01)

    if not ok:
        rospy.logwarn("  retreat staged position-only PD: timeout before stable Cartesian retreat.")
    return ok
...
class Config:
    ...
    approach_xy_speed = 0.022
    approach_z_speed = 0.012
    approach_min_speed_scale = 0.70
    ...
    approach_extra_down_kp = 520.0
    approach_extra_down_force_limit = 10.0
    approach_u_threshold = 46.0
    approach_u_tool_limits = np.array([24.0, 24.0, 22.0])
    approach_tau_norm_limit = 46.0
    approach_start_speed_limit = 0.003
    approach_start_settle_hold = 0.5
    approach_start_settle_timeout = 8.0
    approach_timeout = 240.0
    approach_orientation_prealign_time = 4.0
    approach_orientation_ramp_time = 4.0
    ...
    settle_time = 1.0                  # 初始关节位姿到达后的静置时间, s
    ...
    scan_force_error_soft_limit = 0.0    # legacy vector clip; kept disabled
    scan_force_deadband = 0.0            # 可选控制用力误差死区；默认关闭，避免欠力段补偿不足
    scan_force_error_limit = 0.0         # 可选控制/alpha 用力误差限幅；日志仍保存真实力误差
    scan_sigma_force_limit = 0.0         # 可选力误差泄漏积分限幅；默认关闭
    scan_z_gain_khat_max = 0.0           # 0 表示不裁剪控制用 K_hat

    # 接触调整阶段，不写入实验数据；用于滤掉刚接触瞬态震荡。
    adjustment_min_time = 0.7          # 调整阶段最短持续时间, s
    adjustment_timeout = 6.0           # 调整阶段最长等待时间, s
    ...
    # Phase 3 复位：不再使用官方归位控制器，直接使用 no-RCM effort
    # 控制器做末端笛卡尔位置-only 复位，力误差与力积分均置零。
    retreat_timeout = 28.0
    cartesian_retreat_speed = 0.020
    cartesian_retreat_clearance = 0.050
    cartesian_retreat_min_clearance = 0.030
    cartesian_retreat_z_speed = 0.030
    cartesian_retreat_xy_speed = 0.032
    cartesian_retreat_return_speed = 0.045
    cartesian_retreat_min_speed_scale = 0.70
    cartesian_retreat_follow_tolerance = 0.060
    cartesian_retreat_z_lag_limit = 0.065
    cartesian_retreat_z_tolerance = 0.010
    cartesian_retreat_clear_z_tolerance = 0.020
    cartesian_retreat_xy_tolerance = 0.012
    cartesian_retreat_hold_z_tolerance = 0.020
    cartesian_retreat_min_segment_duration = 3.0
    cartesian_retreat_settle_time = 0.6
    cartesian_retreat_position_tol = 0.012
    cartesian_retreat_velocity_tol = 0.18
    cartesian_retreat_orientation_tol_deg = 8.0
    cartesian_retreat_kp = np.array([52.0, 52.0, 55.0])
    cartesian_retreat_kd = np.array([17.0, 17.0, 16.0])
    cartesian_retreat_final_zone = 0.030
    cartesian_retreat_final_kp_scale = 4.0
    cartesian_retreat_final_kd_scale = 1.2
    cartesian_retreat_u_threshold = 15.0
    cartesian_retreat_u_tool_limits = np.array([10.0, 10.0, 8.0])
    cartesian_retreat_P_ori = 4.0
    cartesian_retreat_D_ori = 2.0
    cartesian_retreat_omega_limit = 0.8
    cartesian_retreat_u_rot_limit = 1.2
    cartesian_retreat_joint_damping = 1.3
    cartesian_retreat_joint_damping_tau_max = 3.5
    cartesian_retreat_tau_norm_limit = 26.0
    cartesian_retreat_tau_rate_limit = 55.0
    cartesian_retreat_position_error_limit = np.array([0.080, 0.080, 0.080])
    cartesian_retreat_velocity_error_limit = np.array([0.18, 0.18, 0.18])
...
def retreat_speed_scale(follow_err, cfg):
    """Slow the retreat reference when the tool lags behind it."""
    ratio = float(follow_err) / max(float(cfg.cartesian_retreat_follow_tolerance), 1e-9)
    if ratio <= 0.5:
        return 1.0
    if ratio >= 1.0:
        return float(cfg.cartesian_retreat_min_speed_scale)
    scale = 1.0 - (ratio - 0.5) / 0.5 * (
        1.0 - float(cfg.cartesian_retreat_min_speed_scale)
    )
    return float(np.clip(scale, cfg.cartesian_retreat_min_speed_scale, 1.0))
...
def run_trial(robot, kin_tool, kin_flange,
    ...
    retreat_tool_home = rs["tool_position"].copy()
    ...
    while not rospy.is_shutdown():
        ...
        e_f_control = float(e_f_vec[cfg.force_axis])
        if cfg.scan_force_deadband > 0.0:
            e_abs = abs(e_f_control)
            if e_abs <= cfg.scan_force_deadband:
                e_f_control = 0.0
            else:
                e_f_control = np.sign(e_f_control) * (
                    e_abs - cfg.scan_force_deadband
                )
        if cfg.scan_force_error_limit > 0.0:
            e_f_control = float(np.clip(
                e_f_control,
                -cfg.scan_force_error_limit,
                cfg.scan_force_error_limit,
            ))
        e_f_vec_control = np.zeros(3, dtype=float)
        e_f_vec_control[cfg.force_axis] = e_f_control
        force_blend = min(1.0, max(0.0, adjust_t / cfg.contact_force_blend_time))
        e_f_vec_ctrl = force_blend * e_f_vec_control
        sigma_f = sigma_f_int.update(e_f_vec_ctrl)
        if cfg.scan_sigma_force_limit > 0.0:
            sigma_f = np.clip(
                sigma_f,
                -cfg.scan_sigma_force_limit,
                cfg.scan_sigma_force_limit,
            )
            sigma_f_int.state = sigma_f.copy()

        F_actual = abs(F_z)
        e_f_scalar = float(e_f_vec_ctrl[cfg.force_axis])
        e_f_dot = (e_f_scalar - prev_ef) / dt; prev_ef = e_f_scalar
        e_r_scalar = np.linalg.norm(tp[:2] - np.array([x_cur, cfg.scan_y]))
        # z-only alpha 只应看力控轴位置误差。x/y 已由 no-RCM 笛卡尔
        # 位置任务固定高权重跟踪；若把 xy 误差喂给 alpha，会在高刚度区
        # 把 z 向 alpha 顶到上限，削弱力误差补偿。
        e_r_alpha = abs(float(e_r1[cfg.force_axis]))
        de_r = (e_r_alpha - prev_er) / dt; prev_er = e_r_alpha
    ...
    while not rospy.is_shutdown() and x_cur < cfg.scan_end_x:
        ...
        e_f_dot = (e_f_scalar - prev_ef) / dt; prev_ef = e_f_scalar
        e_r_scalar = np.linalg.norm(tp[:2] - np.array([x_cur, cfg.scan_y]))
        # z-only alpha 使用 z 轴误差；xy 误差只用于日志和位置控制。
        e_r_alpha = abs(float(e_r1[cfg.force_axis]))
        de_r = (e_r_alpha - prev_er) / dt; prev_er = e_r_alpha
        dK = (K_hat - prev_K) / dt; prev_K = K_hat
    ...
    retreat_ok = retreat_with_position_only_controller(
        robot, kin_tool, kin_flange, ctrl,
        target_pos=retreat_tool_home,
        ref_euler_fixed=ref_euler_fixed,
        cfg=cfg,
    ...
    if not retreat_ok:
        rospy.logwarn(
            "  Position-only Cartesian retreat did not converge; "
            "keeping gravity-compensated effort interface active."
        )
    if not retreat_ok:
        rospy.logwarn("  Retreat did not reach the saved Cartesian home pose within timeout.")
